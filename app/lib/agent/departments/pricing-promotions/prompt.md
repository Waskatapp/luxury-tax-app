...
- `create_discount_code` — percentage-off discount that customers REDEEM with a code at checkout (vs `create_discount` which applies automatically). Use for influencer codes, partner programs, email-list exclusives, first-purchase incentives. The `code` is what customers type; `title` is what the merchant sees in their list. Optional `usageLimit` (total redemptions) and `appliesOncePerCustomer` (one-per-customer). v1 is store-wide only.
